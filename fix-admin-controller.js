// Script to fix the adminController addProduct function
const fs = require('fs');
...
// Find and replace the addProduct function
const addProductStart = content.indexOf('exports.addProduct = async (req, res) => {');
const addProductEnd = content.indexOf('\n};', addProductStart) + 3;

if (addProductStart !== -1 && addProductEnd !== -1) {
    const beforeFunction = content.substring(0, addProductStart);
    const afterFunction = content.substring(addProductEnd);
    
    const newAddProductFunction = `exports.addProduct = async (req, res) => {
    try {
    ...
    const newContent = beforeFunction + newAddProductFunction + afterFunction;
    fs.writeFileSync(adminControllerPath, newContent);
    console.log('✅ Fixed addProduct function in adminController.js');
} else {
    console.log('❌ Could not find addProduct function');
}

console.log('🔧 AdminController fix applied!');
console.log('📝 Changes made:');
console.log('   - Better file handling with Array.isArray() checks');
console.log('   - Improved error handling and logging');
console.log('   - Proper color variant processing');
